...
function openModal() {
  ...
  modalElm.appendChild(closeButton); 
}
...
function closeModal() {
  ...
  if (modalBdy) {
    modalElm.removeChild(modalBdy); 
  }
  
  
  var closeButton = document.getElementsByClassName('close-button')[0];
  if (closeButton) {
    modalElm.removeChild(closeButton);
  }
}
...
for (var i in data[0].groups) {
  mainElm.innerHTML += `<div class='groupDiv' onClick="clickedGroup('${data[0].groups[i].name}')">${data[0].groups[i].name}</div>`;
}